 ...
 * Copied from lib/verse.ts for Convex action bundling (no Next.js deps).
 */
...
async function fetchFromIBibles(
  ...
  return { text, reference: `${name} ${chapter}:${verse}`, source: "bible", language };
}
...
async function fetchQuranEN(): Promise<Verse> {
  ...
  const text = ayah.text.replace(/\[.*?\]/g, "").replace(/\s+/g, " ").trim();
  return {
    text,
...
async function fetchQuranFR(): Promise<Verse> {
  const res = await fetch("https://api.alquran.cloud/v1/ayah/random/fr.hamidullah");
...
const FETCHERS: Record<string, () => Promise<Verse>> = {
  "bible:en": () => fetchFromIBibles("kjv", "en"),
  "bible:fr": () => fetchFromIBibles("lsg", "fr"),
