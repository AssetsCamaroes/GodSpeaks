/**
 * Verse fetching — abstracts all scripture sources into one unified interface.
 *
 * Bible EN (KJV)  → ibibles.net (no auth)
 * Bible FR (LSG)  → ibibles.net (no auth) — replaces scripture.api.bible
 * Quran EN (Asad) → api.alquran.cloud     (no auth)
 * Quran FR (Ham.) → api.alquran.cloud     (no auth)
 *
 * SCRIPTURE_API_KEY is no longer required.
 */
...
// [ibiblesCode, displayName, versesPerChapter[]]
// Used to generate a valid random reference for ibibles.net
...
const BIBLE_BOOKS: [string, string, number[]][] = [
    ...
    ["num","Numbers",[54,34,51,49,31,27,89,26,23,36,35,16,33,45,41,50,13,32,22,29,35,41,30,25,18,65,23,31,39,17,54,42,56,29,34,13]],
    ...
    ["1sa","1 Samuel",[28,36,21,22,12,21,17,22,27,27,15,25,23,52,35,23,58,30,24,42,15,23,29,22,44,25,12,25,11,31,13]],
    ["2sa","2 Samuel",[27,32,39,12,25,23,29,18,13,19,27,31,39,33,37,23,29,33,43,26,22,51,39,25]],
    ["1ki","1 Kings",[53,46,28,34,18,38,51,66,28,29,43,33,34,31,34,34,24,46,21,43,29,53]],
    ...
    ["eze","Ezekiel",[28,10,27,17,17,14,27,18,11,22,25,28,23,23,8,63,24,32,14,49,32,31,49,27,17,21,36,26,21,26,18,32,33,31,15,38,28,23,29,49,26,20,27,31,25,24,23,35]],
    ...
    ["mar","Mark",[45,28,35,41,43,56,37,38,50,52,33,44,37,72,47,20]],
    ["luk","Luke",[80,52,38,44,39,49,50,56,62,42,54,59,35,35,32,31,37,43,48,47,38,71,56,53]],
    ["joh","John",[51,25,36,54,47,71,53,59,41,42,57,50,38,31,27,33,26,40,42,31,25]],
    ...
    ["jam","James",[27,26,18,17,20]],
    ["1pe","1 Peter",[25,25,22,19,14]],
    ["2pe","2 Peter",[21,22,18]],
    ["1jo","1 John",[10,29,24,21,21]],
    ["2jo","2 John",[13]],
    ["3jo","3 John",[14]],
    ["jud","Jude",[25]],
    ["rev","Revelation",[20,29,22,11,14,17,17,13,21,11,19,17,18,20,8,21,18,24,16,15,27,21]],
];

// ─── Random Bible Reference ──────────────────────────────────────────────────

function randomBibleRef(): { code: string; name: string; chapter: number; verse: number } {
    // Weight selection by total verse count so every verse has equal probability
    const totals = BIBLE_BOOKS.map(([,, chapters]) => chapters.reduce((a, b) => a + b, 0));
    const grand  = totals.reduce((a, b) => a + b, 0);

    let roll = Math.random() * grand;
    let bookIndex = 0;
    for (let i = 0; i < totals.length; i++) {
        roll -= totals[i];
        if (roll <= 0) { bookIndex = i; break; }
    }

    const [code, name, chapters] = BIBLE_BOOKS[bookIndex];

    // Random chapter (weighted by verse count)
    const chapterTotal = chapters.reduce((a, b) => a + b, 0);
    let cRoll = Math.random() * chapterTotal;
    let chapterIndex = 0;
    for (let i = 0; i < chapters.length; i++) {
        cRoll -= chapters[i];
        if (cRoll <= 0) { chapterIndex = i; break; }
    }

    const chapter = chapterIndex + 1;
    const verse   = Math.floor(Math.random() * chapters[chapterIndex]) + 1;

    return { code, name, chapter, verse };
}

// ─── ibibles.net Fetcher ─────────────────────────────────────────────────────

async function fetchFromIBibles(
    version: "kjv" | "lsg",
    language: "en" | "fr"
): Promise<Verse> {
    const { code, name, chapter, verse } = randomBibleRef();
    const url = `https://ibibles.net/quote.php?${version}-${code}/${chapter}:${verse}`;

    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) throw new Error(`ibibles.net failed: ${res.status}`);

    const html = await res.text();

    // Strip all HTML tags
    const raw = html.replace(/<[^>]*>/g, " ").replace(/&[a-z]+;/gi, " ");

    // Remove "Bible Quote X:Y " prefix and clean whitespace
    const text = raw
        .replace(/Bible\s+Quote\s+\d+:\d+\s*/i, "")
        .replace(/\s+/g, " ")
        .trim();

    if (!text) throw new Error(`ibibles.net returned empty verse for ${name} ${chapter}:${verse}`);
...
// ─── Bible EN (KJV via ibibles.net) ─────────────────────────────────────────

async function fetchBibleEN(): Promise<Verse> {
    return fetchFromIBibles("kjv", "en");
}

// ─── Bible FR (Louis Segond via ibibles.net) ─────────────────────────────────

async function fetchBibleFR(): Promise<Verse> {
    return fetchFromIBibles("lsg", "fr");
}
